        ...
        return delegate.enqueue(object : Callback<S> {
            override fun onResponse(call: Call<S>, response: Response<S>) {
                ...
                if (response.isSuccessful) {
                    ...
                    if (code == Constants.RESPONSE_CODE_UNAUTHORIZED) {
                        StorageHelper.clearData()
                        if (BaseActivity.handler != null) {
                            val message = Message.obtain()
                            message.what = Constants.RESPONSE_CODE_UNAUTHORIZED
                            BaseActivity.handler?.sendMessage(message)
                        }
                    } else if (code == Constants.RESPONSE_CODE_403) {
                        if (BaseActivity.handler != null) {
                            val message = Message.obtain()
                            message.what = Constants.RESPONSE_CODE_403
                            BaseActivity.handler?.sendMessage(message)
                        }
